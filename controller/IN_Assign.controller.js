sap.ui.define(["Hotline/controller/BaseController",
	...
	return BaseController.extend("Hotline.controller.IN_Assign", {
		formatter: formatter,
		onInit: function () {
			var oRouter = sap.ui.core.UIComponent.getRouterFor(this);
			oRouter.getRoute("in_assign").attachPatternMatched(this._onObjectMatched, this);
		},
		...
		_showAdminTabs: function (oData) {
			...
			this._showTable(oData.results[0].HotlineNum, "IN");
		},
		...
		onSelectTab: function (oEvent) {
			this._setScreenVariables();
			this._showTable(oEvent.getSource().getSelectedKey(), "IN");
		},
		_getData: function (pHotline) {
			...
			if (sTableType.table === "W") {
				...
				aFilters.push(oFilter);
				oFilter = new sap.ui.model.Filter("Country", sap.ui.model.FilterOperator.EQ, "IN");
				aFilters.push(oFilter);
				...
				oFilter = new sap.ui.model.Filter("Quarter", sap.ui.model.FilterOperator.EQ, aHeadData[0].Quarter);
				aFilters.push(oFilter);
				oFilter = new sap.ui.model.Filter("Region", sap.ui.model.FilterOperator.EQ, "IN");
				aFilters.push(oFilter);
				sap.ui.core.BusyIndicator.show();
				this.oModel.read(sPath, {
					filters: aFilters,
			...
			//get list of users
			this._getUsers();
			//check the current state of Hotlines
			this._checkState();
		...
		_getUsers: function () {
			...
			var oFilter = new sap.ui.model.Filter("UserId", sap.ui.model.FilterOperator.EQ, sKey);
			aFilters.push(oFilter);
			...
			this.getView().setModel(oUserModelDe, "user");
		},
			...
			if (sTableType === "M") {
				...
				this.oModel.read("/FinalizeSet", {
					...
					success: function (oData) {
						...
						if (oData.results[0].HotlineNum === "INTINC") {
							that.getView().byId("__msgStrip").setVisible(false);
							that.getView().byId("__msgStrip").setText("");
							that.getView().byId("__aBtn1").setVisible(true);
							that.getView().byId("__aBtn2").setVisible(true);
		...
		saveAll: function (sMessage) {
			...
			if (sTableType.table === "W") {
				...
				for (i = 0; i < oTabModel.results.length; i++) {
					...
					sPath = "WeeklyAssignmentSet(Country='IN',CW='" + sWeek + "',HotlineNum='" + sHotline + "',Year='" + sYear + "')";
					if (i === 0) {
						sFirst = sWeek;
					}
				...
				for (i = 0; i < oTabModel.results.length; i++) {
					...
					sPath = "IN_Assignment_PeerSet(CW='" + sWeek + "',HotlineNum='" + sHotline + "')";
					if (i === 0) {
						sFirst = sWeek;
					}
					if (i === oTabModel.results.length - 1) {
		...
		_lockHotline: function (sHotline, sQtr, sYear) {
			...
			aFltr.push(oFltr);
			oFltr = new sap.ui.model.Filter("Region", sap.ui.model.FilterOperator.EQ, "IN");
			aFltr.push(oFltr);
		...
		//India PRIMARY pop up
		handleINValueHelp: function (oEvent) {
			var sInputValue = oEvent.getSource().getValue();
			this.inputId = oEvent.getSource().getId();
			// create value help dialog
			if (!this._valueHelpDialog_IN) {
				this._valueHelpDialog_IN = sap.ui.xmlfragment(
					"Hotline.view.Dialog",
					this
				);
				this.getView().addDependent(this._valueHelpDialog_IN);
			}
			//create a filter for the binding
			this._valueHelpDialog_IN.getBinding("items").filter([new sap.ui.model.Filter(
				"Name",
				sap.ui.model.FilterOperator.Contains, sInputValue
			)]);

			// open value help dialog filtered by the input value
			this._valueHelpDialog_IN.open(sInputValue);
		},
		...
		backuphandleINValueHelp: function (oEvent) {
			var sInputValue = oEvent.getSource().getValue();
			this.inputId = oEvent.getSource().getId();
			// create value help dialog
			if (!this._valueHelpDialog_IN2) {
				this._valueHelpDialog_IN2 = sap.ui.xmlfragment(
					"Hotline.view.BackupDialog",
					this
				);
				this.getView().addDependent(this._valueHelpDialog_IN2);
			}
			// create a filter for the binding
			this._valueHelpDialog_IN2.getBinding("items").filter([new sap.ui.model.Filter(
				"Name",
				sap.ui.model.FilterOperator.Contains, sInputValue
			)]);

			// open value help dialog filtered by the input value
			this._valueHelpDialog_IN2.open(sInputValue);
		},
