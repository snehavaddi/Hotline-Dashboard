sap.ui.define(["Hotline/controller/BaseController",
	...
	return BaseController.extend("Hotline.controller.DE_Assign", {
		formatter: formatter,
		onInit: function() {
			var oRouter = sap.ui.core.UIComponent.getRouterFor(this);
			oRouter.getRoute("de_assign").attachPatternMatched(this._onObjectMatched, this);
		},
		_onObjectMatched: function() {
			...
			var oHModel = new sap.ui.model.json.JSONModel({
				key: ''
			});
			this.getView().setModel(oHModel, "selected");
		...
		_showAdminTabs: function(oData) {
			...
			this._showTable(oData.results[0].HotlineNum, "DE");
		},
		_showTable: function(pHotline, pCountry) {
			var aFilters = [];
			var that = this;
			var aData = this.getView().getModel("selected").getData();
			aData.key = pHotline;
		...
		onSelectTab: function(oEvent) {
			this._setScreenVariables();
			this._showTable(oEvent.getSource().getSelectedKey(), "DE");
		},
		_getData: function(pHotline) {
			...
			if (sTableType.table === "W") {
				...
				aFilters.push(oFilter);
				oFilter = new sap.ui.model.Filter("Country", sap.ui.model.FilterOperator.EQ, "DE");
				aFilters.push(oFilter);
				...
				oFilter = new sap.ui.model.Filter("Quarter", sap.ui.model.FilterOperator.EQ, aHeadData[0].Quarter);
				aFilters.push(oFilter);
				oFilter = new sap.ui.model.Filter("Region", sap.ui.model.FilterOperator.EQ, "DE");
				aFilters.push(oFilter);
				sap.ui.core.BusyIndicator.show();
				this.oModel.read(sPath, {
					filters: aFilters,
		...
		_getUsers: function() {
			...
			var oFilter = new sap.ui.model.Filter("UserId", sap.ui.model.FilterOperator.EQ, sKey + "_GER");
			aFilters.push(oFilter);
			...
			this.getView().setModel(oUserModelDe, "ger");
		},
		...
		_lockHotline: function(sHotline, sQtr, sYear) {
			...
			aFltr.push(oFltr);
			oFltr = new sap.ui.model.Filter("Region", sap.ui.model.FilterOperator.EQ, "DE");
			aFltr.push(oFltr);
		...
		onApproveDialog: function() {
			var sKey = this.getView().getModel("selected").getData().key;
			var oRouter = sap.ui.core.UIComponent.getRouterFor(this);
			var dialog = new sap.m.Dialog({
				title: 'Confirm',
				type: 'Message',
				content: new sap.m.Text({
					text: 'Are you sure you want to navigate to Manage Page? All unsaved changes will be lost.'
				}),
				beginButton: new sap.m.Button({
					text: 'Yes',
					press: function() {
