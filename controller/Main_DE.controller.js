sap.ui.define(["Hotline/controller/BaseController",
	...
	return BaseController.extend("Hotline.controller.Main_DE", {
		formatter: formatter,
		onInit: function () {
			var oRouter = sap.ui.core.UIComponent.getRouterFor(this);
			oRouter.getRoute("main_DE").attachPatternMatched(this._onObjectMatched, this);
		},
		_onObjectMatched: function (oEvent) {
			...
			this.oModel.read("/ListSet", {
				success: function (oData) {
					...
					that.getView().byId("hList").setModel(that.oListModel);
			...
			//user target
			var oUserTargetModel = new sap.ui.model.json.JSONModel();
			this.oModel.read("/UserTargetSet", {
				success: function (oData) {
					if (oData.results.length > 0) {
						oUserTargetModel.setData({
							show: true,
			...
			this.getView().setModel(oUserTargetModel, "userTarget");
			...
			this.getView().byId("saveBtnMDe").setVisible(true);
		...
		createTable: function (oListModel, that) {
			...
			for (var i = 0; i < aData.length; i++) {
				...
				if (aData[i].Overlapping <= 1) {
					if (aData[i].HotlineNum === "AOF") {
						var oCell = new sap.m.SegmentedButton({
							selectedKey: {
								path: sPath
							},
							select: function (oControlEvent) {
								that.compareResults(oControlEvent, "selectedKey");
							}
						});
						//create 3 buttons
						var oBtn;
						oBtn = new sap.m.SegmentedButtonItem({
							text: "Yes",
							key: "YES",
							enabled: {
								path: sPathE
						...
						var oCell = new sap.m.Switch({
							...
							change: function (oControlEvent) {
								that.compareResults(oControlEvent, "state");
							}
						});
							...
							for (var j = 0; j < temp1.length; j++) {
								if (temp1[j].HotlineKey === "AOF") {
									if (i == temp1[j].CW) {
										...
										if (temp1[j].Maybe == "1") {
											mData[temp1[j].HotlineKey] = "MAYBE";
										}
									}
								} else {
									if (i == temp1[j].CW) {
		...
		compareDailyResults: function (oControlEvent) {
			...
			var oRow = this.getView().byId("pTable").getModel().getProperty(sPath);
			...
			var oListModel = this.getView().byId("hList").getModel();
			var aData = oListModel.getData().results;
			...
			this.getView().byId("pTable").getModel().refresh(true);
		},
		compareResults: function (oControlEvent, type) {
			//same hotline check
			var firstState, secondState, strTemp, firstEnable, secondEnable;
			var s1;
			var sPath = oControlEvent.getSource().getBindingContext().sPath;
			try {
				var stateText = oControlEvent.getSource().getBindingInfo(type).parts[0].path;
			} catch (e) {
				//ignore
			}
			...
			if (stateText.slice(-1) === "B") { //backup is newly selected, so revert backup if both are true
				...
				if (secondState === "MAYBE" || secondState === "NO") {
					secondState = false;
				} else if (secondState === "YES") {
					secondState = true;
				}
				s1 = stateText + "_E";
				firstEnable = oRow[s1];
			...
			//Backlog - 1807, only for one "YES"
			if (firstState === "YES" && firstEnable) {
				var aTable = this.getView().byId("pTable").getModel().getData();
				for (var i = 0; i < aTable.length; i++) {
					var aCurrentRow = aTable[i];
					if (aCurrentRow[stateText] === "YES" && oRow.CW != aCurrentRow.CW) {
						oRow[stateText] = "NO";
						sap.m.MessageToast.show("Only one 'YES' is allowed per cycle");
					}
				}
			}
		...
		saveAll: function () {
			var aTableData = this.getView().byId("pTable").getModel().getData();
			var oListModel = this.getView().byId("hList").getModel();
			var aData = oListModel.getData().results;
			var batchChanges = [];
			var iCtr = 0;
			...
			for (var i = 0; i < aData.length; i++) {
				if (aData[i].Overlapping <= "1") {
					for (var j = 0; j < aTableData.length; j++) {
						...
						var temp = aTableData[j];
						var temp1 = aInitData[j];
						sValue = temp[sHotlineKey];
						sOldValue = temp1[sHotlineKey];
						var changedBy = sHotlineKey + "_E";
						...
						if (sValue !== sOldValue) {
							...
							oEntry.Quarter = "";
							oEntry.Maybe = maybe;
							sPath = "Choice_DESet(CW='" + sCW + "',HotlineKey='" + sHotlineKey + "')";
					...
					for (j = 0; j < aTableData.length; j++) {
						...
						for (var k = 1; k <= 5; k++) {
							...
							if (changedBy === true && sValue !== sOldValue) {
								...
								oEntry.Quarter = "";
								oEntry.Country = "DE"; //india from this page
		...
		showHelp: function () {
			if (!this._helpDialog) {
				this._helpDialog = sap.ui.xmlfragment("Hotline.view.PrefHelp", this);
			}
			this.getView().addDependent(this._helpDialog);
			this._helpDialog.open();
		},
		closeHelpPref: function () {
			this._helpDialog.close();
		},
		destroyPrefHelp: function () {
