sap.ui.define(["Hotline/controller/BaseController",
	...
	return BaseController.extend("Hotline.controller.Main", {
		formatter: formatter,
		onInit: function() {
		...
		_onObjectMatched: function(oEvent) {
			...
			this.oModel.read("/ListSet", {
				success: function(oData) {
					...
					that.getView().byId("dragableList").setModel(that.oListModel);
		...
		createTable: function(oListModel, that) {
			...
			var oTable = this.getView().byId("dropableTable");
		...
		compareDailyResults: function(oControlEvent) {
			...
			var oRow = this.getView().byId("dropableTable").getModel().getProperty(sPath);
			...
			var oListModel = this.getView().byId("dragableList").getModel();
			var aData = oListModel.getData().results;
			...
			this.getView().byId("dropableTable").getModel().refresh(true);
		},
		...
		saveAll: function() {
			var aTableData = this.getView().byId("dropableTable").getModel().getData();
			var oListModel = this.getView().byId("dragableList").getModel();
			var aData = oListModel.getData().results;
			var batchChanges = [];
			var iCtr = 0;
				...
				if (aData[i].Overlapping <= "1") {
					for (var j = 0; j < aTableData.length; j++) {
						...
						if (changedBy === true && sValue !== sOldValue) {
							...
							oEntry.Quarter = "";
							sPath = "ChoiceSet(CW='" + sCW + "',HotlineKey='" + sHotlineKey + "')";
					...
					for (j = 0; j < aTableData.length; j++) {
						...
						for (var k = 1; k <= 5; k++) {
							...
							if (changedBy === true && sValue !== sOldValue) {
								...
								oEntry.Quarter = "";
								oEntry.Country = "IN"; //india from this page
